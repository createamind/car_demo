...
class DDPG(object):
    def __init__(self):
        ...
        self.S_ = tf.placeholder(tf.float32, [None, CAMERA_CHANNEL, CAMERA_H, CAMERA_W, CAMERA_CHANNEL], 's_')
        ...
        grads = opt.compute_gradients(a_loss, var_list=self.ae_params)
    ...
    def _build_cnn(self, s):
        ...
        for image in cameras:
            with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu):
                l = slim.conv2d(image, 32, 5, scope='conv0')
                l = slim.max_pool2d(l, 2, scope='pool0')
                l = slim.conv2d(l, 32, 5, scope='conv1')
                l = slim.max_pool2d(l, 2, scope='pool1')
                l = slim.conv2d(l, 64, 4, scope='conv2')
                l = slim.max_pool2d(l, 2, scope='pool2')
                l = slim.conv2d(l, 64, 3, scope='conv3')
            outputs.append(l)
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            return tf.concat([steering * 0.5, throttle], axis=-1, name='scaled_a')
